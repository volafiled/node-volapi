...
class Room extends EventEmitter {
   ...
   * @param {Room} [other] Another room from which to take user information
   */
  constructor(id, nick, other) {
    id = parseId(id);
    if (!id) {
  ...
  async connect() {
    ...
    debug(this.config);
    const url = `wss://${this.config.site}/api/?room=${this.id}&cs=${this.config.checksum2}&nick=${this.nick}&rn=${Math.random()}`;
    debug(url);
  ...
  async _getUploadKey() {
    for (;;) {
      const params = await this.callREST("getUploadKey", {
        name: this.nick,
        room: this.id,
        c: ++this._uploadCount
      });
  ...
  async ensureConfig() {
    ...
    let config = await fetch(this.url, {headers: this.headers});
    config = await config.text();
    config = config.replace("\n", "");
    config = config.replace(
      /(\w+):(?=([^"\\]*(\\.|"([^"\\]*\\.)*[^"\\]*"))*[^"]*$)/,
      "\"$1\":");
    config = config.match(/config\s*=\s*({.+});/);
    if (!config) {
      throw new VolaError("Failed to get config");
    }
    Object.assign(this.config, JSON.parse(config[1]), {loaded: true});
